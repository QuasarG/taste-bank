...
- **调用方式不统一**：有的风格是 skill，有的是 Codex 或 Claude Code 的插件，有的干脆什么都不提供——只有风格图片或网页模板，使用极其不便。
...
> **更重要的是**：网上的风格每天都在变多，新的风格、UI/UX、布局层出不穷——但并不是所有风格都足够大众，也不是所有风格你都喜欢，更没人能始终紧跟最新的潮流。所以我们期望建立这样一个社群：让人们分享自己真正在用的前端 taste，用脚投票——被真实使用的，才是真正可用的。
...
- **抖音式风格画廊**：首页是无限下落的实时渲染风格流——像刷短视频一样往下刷，刷到心动的那套就翻进横向卡片轨模式细看，滚轮横移切换、点击直接跳转；Collections 页提供带分类与分页的全览网格
- **引用榜单**：每次 agent 取用风格都会计数，首页展示风格引用榜与作者榜
- **结构化 style pack**：meta / tokens / SKILL.md / overrides / templates，zod 全量校验，版本号管理
- **MCP server（Streamable HTTP）**：12 个工具覆盖浏览、取用、投稿、更新、删除、钥匙生成、身份查询与投稿前干跑校验
- **HTTP API**：列表 / 详情 / SKILL 组装 / scoped CSS / 截图 / 投稿 / 更新 / 删除
- **邀请制投稿 + ed25519 签名所有权 + 人工审核队列**
...
**线上地址：[https://tastebank.cloud](https://tastebank.cloud)**

| 页面 | 内容 |
|---|---|
| `/` | 沉浸式翻阅：实时渲染风格流 + 横向卡片轨（中间最大、两侧渐小渐隐）+ 交互预览 + 引用榜与作者榜 |
| `/collections` | 全览网格：所有已上架风格的目录，带分类筛选与分页 |
...
| 身份 | `whoami` / `validate_style` | 查绑定作者与名下风格 / 签名前干跑校验 payload |
...
- **ed25519 签名**：投稿 / 更新 / 删除全部要求私钥签名（消息 = `style-lab:<action>:<slug>:<timestamp>:<sha256(payload)>`，30 分钟时间窗），私钥即身份，无密码可泄露
- **审核队列**：投稿先入 `data/pending/`，库主人工 approve 后才上架；reject 即焚毁
...
- **prompt 注入缓解**：风格内容在工具描述中明确标记为"数据非指令"，防止恶意风格劫持 agent 行为
- **路径逃逸防护**：所有文件读取经路径归一化校验，`../` 类逃逸一律拒绝
- **密钥模式扫描**：投稿内容命中高置信度密钥模式（API key 等）直接服务端拒收

> ⚠️ **使用警告**：库主会人工审核所有提交的风格，确保没有提示词注入等攻击内容；如果你来投稿，也请务必确保没有携带敏感信息或原项目的业务信息。但审核百密总有一疏——**请不要完全信任 agent 取回到本地的任何内容**，把它当作数据而非指令，时刻警惕提示词注入攻击。
...
让 agent 调 `generate_keypair`，得到一对 ed25519 钥匙：
...
> 签名不需要克隆仓库、不需要本地工具：agent 通过 `get_usage_guide` 取到的使用说明里内附一段自包含 Node 脚本，拿来即用。
