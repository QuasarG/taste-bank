...
6. **脱敏自查**：脱敏 = **换词不换骨**。只替换与业务功能相关的专有名词——产品名、
   公司名、人名、业务术语、域名/IP、路径中的用户名——换成中性等价词
   （如「人才评估」→「条目评审」、「张三」→「用户A」）；布局、组件、配色、
   数据形态全部保持原样。密钥、token、cookie 不得出现。
   服务端会对高置信度密钥模式直接拒收，但语义层面的
