...
5. **做模板快照**：取一个代表性页面，改写为自包含 HTML（内联 CSS、无 script、
   无外部依赖），可直接在浏览器打开，作为 `templates/page.html`。
   **快照中的文案数据一律虚构**，不带真实业务内容。
6. **脱敏自查**：把投稿内容当作会公开的信息处理——真实项目名、公司名、人名、
   域名/IP、内部链接、路径中的用户名，全部替换为占位符；确认没有密钥、token、
   cookie、内网地址残留。服务端会对高置信度密钥模式直接拒收，但语义层面的
   隐私（这项目属于哪家公司、谁的名字）只有你把得住，漏了就是事故。
