...
1. **邀请码**：投稿需要邀请码（向库主索取），由用户在 MCP 配置中注入——
   HTTP 模式放 `headers` 的 `x-invite-code`，stdio 模式放 `env` 的 `STYLE_LAB_INVITE`。
   邀请码与公钥一码一身份绑定
2. 钥匙的获取与持久化（**会话失忆是最大的坑**，严格按顺序来）：
   - **先查** `~/.style-lab/private.key` 是否存在——存在就直接用，
     **绝不重复生成**（owner.key 里存的是旧公钥，新私钥对不上 = 旧风格永久失控）
   - 不存在才生成，三选一：
   ...
   - 生成后**立即持久化**：私钥写入 `~/.style-lab/private.key`（`chmod 600`），
     公钥写入 `~/.style-lab/public.key`，并提醒用户备份私钥、**不要提交进任何 git 仓库**
   - 换钥匙的正路：用旧私钥做一次 `update_style`，payload 里的 `ownerPubkey` 填新公钥
3. 投稿时在 `ownerPubkey` 字段带上公钥，完成所有权登记
4. 更新/删除时签名：消息 = `style-lab:<update|delete>:<slug>:<timestamp>:<sha256(payload)>`，
   update 的 payload 为 pack JSON 字符串，delete 为空串。仓库里有 `npm run sign -- <私钥> <action> <slug> [payload文件]`；
   无仓库时用下面这段自包含脚本（只需 Node，无需仓库）：

```js
// sign.mjs 用法: node sign.mjs <私钥base64> <update|delete> <slug> [payload文件]
...
5. 调 `update_style` / `delete_style`（或 HTTP PUT/DELETE）。未登记 owner 的风格不可管理
