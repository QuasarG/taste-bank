...
| `whoami` | 凭邀请码查身份：绑定作者名、名下风格（含审核队列）——投稿前先查 author 该叫什么 |
| `validate_style` | 干跑校验：只校验不提交、无需签名邀请码，schema 问题在签名前清完 |
...
4. 签名（投稿、更新、删除都要）：消息 = `style-lab:<submit|update|delete>:<slug>:<timestamp>:<sha256(payload)>`，
   ...
   **走文件式流程，禁止用 echo / heredoc / 内联 JSON 拼长命令**（长嵌套命令极易写崩）：
   ...
   b. 用下面这段自包含脚本（只需 Node，无需任何仓库）生成签名，命令本身很短，
      ...
      **私钥默认从 `~/.style-lab/private.key` 读取，不要把私钥放进命令行参数**
      （`ps` 和 shell history 都会泄露）
   c. 调 `submit_style` / `update_style` 时，payload 参数直接传临时文件的完整内容
      （工具参数是结构化字符串，不涉及任何 shell 转义）
   d. 调用成功后**删除临时文件**

```js
// sign.mjs 用法: node sign.mjs <submit|update|delete> <slug> [payload文件路径]
// 私钥自动读 ~/.style-lab/private.key（Windows: %USERPROFILE%\.style-lab\private.key）
import crypto from 'node:crypto';
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';
const [, , action, slug, file] = process.argv;
const payload = file ? fs.readFileSync(file, 'utf8') : '';
const priv = fs.readFileSync(path.join(os.homedir(), '.style-lab', 'private.key'), 'utf8').trim();
...
5. 调 `submit_style` / `update_style` / `delete_style`（或 HTTP POST/PUT/DELETE）。
   未登记 owner 的风格不可管理。提交成功会返回 `payloadHash`（服务端收到的 payload
   原文 sha256）——与你本地的 sha256 对一下，不一致说明传输中被改动了
6. **投稿不等于上架**：投稿成功后进入审核队列，等库主 approve 后
   `list_styles` 才能看到。投稿后提醒用户去审核

## HTTP API（与 MCP 等价）

**HTTP API 在网站端口（4321），MCP 端口（3100）只挂 `/mcp`——不要在 3100 上试 `/api/*`。**
...
头 `x-invite-code` 必填，签名走 `x-timestamp` / `x-signature` 头）。
不支持 MCP 的 agent 直接 curl 即可。**大 payload 优先走 HTTP**：
`curl -X POST http://<host>:4321/api/styles.json -H 'content-type: application/json' \
 -H 'x-invite-code: <码>' -H 'x-timestamp: <ts>' -H 'x-signature: <sig>' \
 --data-binary @/tmp/style-lab-<slug>.json`
整条链路（签名 + 发送）可以在一个脚本里毫秒级完成，不经过 LLM 上下文手递。
...
字体用系统字体栈近似表达）。
**单页约束**：一屏 16:9（如 1600×900）内完整呈现、禁止页内滚动；
放不下拆 `page.html`、`page2.html`……
**自查方法**：用浏览器或截图工具以 1600×900 渲染模板文件，确认内容完整且无滚动条。
**体积软上限 ≈40KB**：快照要保留丰富度，但装饰性内容（长渐变、重复占位文本、
base64 图片）一律精简——超过 40KB 的 pack 走 MCP 手递几乎必然失败，见「大 pack 投稿」。
...
1. 先调 `validate_style` 干跑校验，把 schema 问题一次性清完（不烧签名窗口）
2. 用**文件写入工具**把 payload JSON 写到系统临时目录
   （`/tmp/style-lab-<slug>.json` 或 `%TEMP%`），禁止写进项目目录
3. 用「管理自己的风格」第 4 步的自包含脚本签名：
   `node sign.mjs submit <slug> /tmp/style-lab-<slug>.json`
   （私钥自动读 `~/.style-lab/private.key`）
4. **签名后立刻**调 `submit_style`：`payload` 传临时文件完整内容，
   `timestamp` / `signature` 传脚本输出。**顺序必须是"一切就绪 → 最后签名 → 立即提交"**；
   任何一次重试都要重新签名
5. 核对返回的 `payloadHash` 与本地 sha256 一致；**删除临时文件**，
   并告诉用户"已进入审核队列，等库主 approve 后上架"
...
| 作者名已被占用 | 调 `whoami` 确认本名，或读 `~/.style-lab/author` |
| version 必须递增 | `get_style` 查现有版本，+0.0.1 |
| timestamp 超窗 | 重新签名立即提交；正确顺序见 Phase 3 第 4 步 |
| 签名验证失败 | payload 与签名时不一致（哪怕差一个字符）或私钥不匹配；核对返回的 payloadHash |

## 大 pack 投稿（>10KB 必读）

LLM 上下文手递是大 payload 的头号杀手：Read 工具单行有 2000 字符截断，
逐 token 生成几十 KB 进工具参数既慢又容易拼错——拼错一个字符签名就对不上。
所以：

1. **优先走 HTTP API（网站 4321 端口）**：写一个脚本完成「读文件 → 签名 →
   `curl --data-binary @file` 发送」，毫秒级完成，payload 完全不经过对话上下文
2. 只能走 MCP 时：payload 控制在 **40KB 以内**，超出就先压缩模板
   （精简装饰性 CSS、重复占位文本、长渐变；保渲染的换行可改 `<br>`）
3. **顺序铁律**：一切就绪 → 最后签名 → 立即提交；任何重试都必须重新签名
4. 本地核对：提交前用 `crypto.createHash('sha256').update(payload)` 自算，
   与返回的 `payloadHash` 比对，不一致说明传输被改动
