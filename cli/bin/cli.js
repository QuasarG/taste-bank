...
const COMMANDS = {
  ...
  css: () => import('../src/commands/css.mjs').then((m) => m.cmdCss(rest)),
...
function printHelp() {
  const lines = [
    ...
    '    taste-bank skill <slug>           输出某风格的完整 SKILL.md（粘给 agent）',
    '    taste-bank css <slug>             输出某风格的 scoped CSS 变量块',
    '    taste-bank use <slug> [--as X]    把风格落地成项目规则文件',
    '                                      --as agents|claude|skill（默认 skill）',
    '    taste-bank doctor                 体检：CLI/网络/身份/skill 健康度',
    '',
    '  选项：',
    '    --version, -v                     打印版本号',
    '    --help, -h                        本帮助',
