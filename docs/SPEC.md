...
单页约束：每个模板页必须按**固定画布 1600×900**（16:9）绘制完整内容，**禁止页内滚动**。
展示端（详情页 iframe、首页卡片轨）会把这 1600×900 的画布等比缩放进各自响应式容器，
所以你**必须用固定 1600×900 的根容器**，内容在它内部排布——不要自己做响应式，
也不要让内容溢出 1600×900（溢出部分会被裁切，看不见）。

推荐骨架（所有尺寸基于这个固定画布，用 px 单位）：
```html
<style>
  html, body { margin: 0; overflow: hidden; }
  .stage { width: 1600px; height: 900px; /* 你的全部内容画在这里面 */ }
</style>
<div class="stage"> … </div>
```

一页 1600×900 放不下就拆多页：`page.html`、`page2.html`……展示端会为多页模板提供 ◀ ▶ 翻页按钮。
