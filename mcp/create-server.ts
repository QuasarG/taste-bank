...
export function createStyleLabServer(): McpServer {
  ...
  server.registerTool(
    'submit_style',
    {
      title: '投稿新风格',
      description:
        '提交一套新风格 pack：meta + tokens + skill 全文 + 可选 overrides + 可选模板文件（至少含一个 .html）。校验通过后立即被网站与 MCP 收录。若是从现有项目提炼风格，先通过 get_usage_guide 阅读「提炼并投稿」工作流再动手，并按要求完成脱敏（替换真实项目名/域名/密钥/业务数据；高置信度密钥模式会被服务端直接拒收）。校验失败会报错并说明原因，修正后重试，不要绕过校验。',
      inputSchema: {
